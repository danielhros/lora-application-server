...
We are using two docker-compose files. The `docker-compose.dev.yml` for develop mode and `docker-compose.yml` for production mode, both located in root directory and. Both compose files loads environment variables specified in `.env` file.
